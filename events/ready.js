module.exports = async (client) => {
  ...
  for (const guild of leftGuilds) {
    ...
    client.logger.info(`Nino has left ${guild.guild_name}`);
  }

  client.logger.info('Nino is now online');
  client.logger.info(`Nino is running on ${client.guilds.cache.size} server(s)`);
};
